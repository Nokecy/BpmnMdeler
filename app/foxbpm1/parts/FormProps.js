...
module.exports = function(group, element, bpmnFactory) {
  ...
  group.entries.push(entryFactory.textField({
    ...
    set: function(element, values, node) {
      ...
      return cmdHelper.updateBusinessObject(element, bo, { 'foxbpm:formKey': formKey });
    }
  }));
  ...
  var formFieldsEntry = extensionElements(element, bpmnFactory, {
    ...
    createExtensionElement: function(element, extensionElements, value) {
      ...
      if (!formData) {
        formData = elementHelper.createElement('foxbpm:FormData', { fields: [] }, extensionElements, bpmnFactory);
      ...
      var field = elementHelper.createElement('foxbpm:FormField', { id: value }, formData, bpmnFactory);
    ...
    removeExtensionElement: function(element, extensionElements, value, idx) {
      var formData = getExtensionElements(getBusinessObject(element), 'foxbpm:FormData')[0],
  ...
  var formBusinessKeyFormFieldEntry = entryFactory.selectBox({
    ...
    get: function(element, node) {
      ...
      var bo = getBusinessObject(element);
      var formDataExtension = getExtensionElements(bo, 'foxbpm:FormData');
  ...
  group.entries.push(entryFactory.comboBox({
    ...
    set: function(element, values, node) {
      var selectedFormField = getSelectedFormField(element, node),
          formData = getExtensionElements(getBusinessObject(element), 'foxbpm:FormData')[0],
          commands = [];

      if (selectedFormField.type === 'enum' && values.type !== 'enum') {
        // delete foxbpm:value objects from formField.values when switching from type enum
  ...
  group.entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      ...
      var enumValue = elementHelper.createElement('foxbpm:Value', { id: id, name: undefined },
  ...
  group.entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      ...
      if (!validation) {
        ...
        validation = elementHelper.createElement('foxbpm:Validation', {}, getBusinessObject(element), bpmnFactory);

        commands.push(cmdHelper.updateBusinessObject(element, formField, { 'validation': validation }));
      }

      var newConstraint = elementHelper.createElement('foxbpm:Constraint',
    ...
    removeElement: function(element, node, idx) {
      ...
      if (constraints.length === 1) {
        // remove foxbpm:validation if the last existing constraint has been removed
  ...
  // [FormData] foxbpm:properties table
