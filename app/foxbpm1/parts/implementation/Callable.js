...
var attributeInfo = {
  bpmn: {
    ...
    binding: 'foxbpm:calledElementBinding',
    version: 'foxbpm:calledElementVersion',
    tenantId: 'foxbpm:calledElementTenantId'
  },

  cmmn: {
    element: 'foxbpm:caseRef',
    binding: 'foxbpm:caseBinding',
    version: 'foxbpm:caseVersion',
    tenantId: 'foxbpm:caseTenantId'
  },

  dmn: {
    element: 'foxbpm:decisionRef',
    binding: 'foxbpm:decisionRefBinding',
    version: 'foxbpm:decisionRefVersion',
    tenantId: 'foxbpm:decisionRefTenantId'
  }
};
...
function getCamundaInWithBusinessKey(element) {
  ...
  var camundaInParams = extensionElementsHelper.getExtensionElements(bo, 'foxbpm:In');
  if (camundaInParams) {
...
module.exports = function(element, bpmnFactory, options) {
  ...
  function getDelegateVariableMappingType(element) {
    ...
    var boVariableMappingClass = bo.get('foxbpm:variableMappingClass'),
        boVariableMappingDelegateExpression = bo.get('foxbpm:variableMappingDelegateExpression');
  ...
  entries.push(entryFactory.selectBox({
    ...
    get: function(element, node) {
      var bo = getBusinessObject(element);
      return {
        mapDecisionResult: bo.get('foxbpm:mapDecisionResult') || 'resultList'
      };
    },

    set: function(element, values, node) {
      return cmdHelper.updateProperties(element, {
        'foxbpm:mapDecisionResult': values.mapDecisionResult || 'resultList'
      });
    },

    hidden: function(element, node) {
      var bo = getBusinessObject(element);
      var resultVariable = bo.get('foxbpm:resultVariable');
  ...
  entries.push(entryFactory.selectBox({
    ...
    set: function(element, values, node) {
      ...
      if (delegateVariableMappingType === 'variableMappingClass') {
        props['foxbpm:variableMappingClass'] = '';
      }
      else if (delegateVariableMappingType === 'variableMappingDelegateExpression') {
        props['foxbpm:variableMappingDelegateExpression'] = '';
      }
  ...
  entries.push(entryFactory.textField({
    ...
    get: function(element, node) {
      ...
      if (type === 'variableMappingClass') {
        label = 'Class';
        delegateVariableMapping = bo.get('foxbpm:variableMappingClass');
      }
      else if (type === 'variableMappingDelegateExpression') {
        label = 'Delegate Expression';
        delegateVariableMapping = bo.get('foxbpm:variableMappingDelegateExpression');
      }
    ...
    set: function(element, values, node) {
      ...
      var attr = 'foxbpm:' + getDelegateVariableMappingType(element);
