...
function isScript(elem) {
  return is(elem, 'foxbpm:Script');
}

function isList(elem) {
  return is(elem, 'foxbpm:List');
}

function isMap(elem) {
  return is(elem, 'foxbpm:Map');
}
...
module.exports = function(element, bpmnFactory, options) {
  ...
  entries.push(entryFactory.selectBox({
    ...
    set: function(element, values, node) {
      ...
      if (parameterType === 'script') {
        properties.definition = createParameterTypeElem('foxbpm:Script');
      }
      else if (parameterType === 'list') {
        properties.definition = createParameterTypeElem('foxbpm:List');
      }
      else if (parameterType === 'map') {
        properties.definition = createParameterTypeElem('foxbpm:Map');
      }
  ...
  entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      var bo = getSelected(element, node);
      var newValue = createElement('foxbpm:Value', bo.definition, bpmnFactory, { value: undefined });
  ...
  entries.push(entryFactory.table({
    ...
    addElement: function(element, node) {
      var bo = getSelected(element, node);
      var newEntry = createElement('foxbpm:Entry', bo.definition, bpmnFactory, { key: undefined, value: undefined });
