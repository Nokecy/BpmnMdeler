...
var CAMUNDA_PROPERTY_TYPE = 'foxbpm:property',
    CAMUNDA_INPUT_PARAMETER_TYPE = 'foxbpm:inputParameter',
    CAMUNDA_OUTPUT_PARAMETER_TYPE = 'foxbpm:outputParameter',
    CAMUNDA_IN_TYPE = 'foxbpm:in',
    CAMUNDA_OUT_TYPE = 'foxbpm:out',
    CAMUNDA_IN_BUSINESS_KEY_TYPE = 'foxbpm:in:businessKey',
    CAMUNDA_EXECUTION_LISTENER_TYPE = 'foxbpm:executionListener';
...
function getPropertyValue(element, property) {
  ...
  if (bindingType === CAMUNDA_PROPERTY_TYPE) {
    camundaProperties = findExtension(bo, 'foxbpm:Properties');
  ...
  if (IO_BINDING_TYPES.indexOf(bindingType) !== -1) {
    inputOutput = findExtension(bo, 'foxbpm:InputOutput');
  ...
  if (bindingType === CAMUNDA_EXECUTION_LISTENER_TYPE) {
    var executionListener = findExtension(bo, 'foxbpm:ExecutionListener');
...
function setPropertyValue(element, property, value, bpmnFactory) {
  ...
  if (bindingType === CAMUNDA_PROPERTY_TYPE) {
    ...
    camundaProperties = findExtension(extensionElements, 'foxbpm:Properties');

    if (!camundaProperties) {
      camundaProperties = bpmnFactory.create('foxbpm:Properties');
  ...
  // foxbpm:inputParameter
  // foxbpm:outputParameter
  var inputOutput,
      existingIoParameter,
      newIoParameter;
  ...
  if (IO_BINDING_TYPES.indexOf(bindingType) !== -1) {
    ...
    inputOutput = findExtension(extensionElements, 'foxbpm:InputOutput');

    // create inputOutput element, if it do not exist (yet)
    if (!inputOutput) {
      inputOutput = bpmnFactory.create('foxbpm:InputOutput');
