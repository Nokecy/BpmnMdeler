...
module.exports = function(element, bpmnFactory, options) {
  ...
  var asyncBeforeEntry = entryFactory.checkbox({
    ...
    set: function(element, values) {
      ...
      var props = {
        'foxbpm:asyncBefore': asyncBefore,
        'foxbpm:async': false
      };

      var commands = [];
      if (!isAsyncAfter(bo) && !asyncBefore) {
        props = assign({ 'foxbpm:exclusive' : true }, props);
  ...
  var exclusiveEntry = entryFactory.checkbox({
    ...
    set: function(element, values) {
      var bo = getBusinessObject(element);
      return cmdHelper.updateBusinessObject(element, bo, { 'foxbpm:exclusive': !!values.exclusive });
    },
